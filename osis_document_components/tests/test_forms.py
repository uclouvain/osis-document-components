...
class FormTestCase(TestCase):
    def setUp(self):
        ...
        mock_remote_token = patch('osis_document_components.services.get_remote_token', return_value='a:token')
    ...
    def test_initial_uuid(self):
        ...
        self.assertNotIn(random_uuid, form.as_p(), msg="The uuid should not be exposed")
    ...
    def test_wrong_upload(self):
        ...
        with patch('osis_document_components.services.get_remote_metadata') as get_remote_metadata:
    ...
    def test_check_file_min_count(self):
        ...
        self.assertFalse(form.is_valid(), msg="Nombre de fichier min. pas respecté")
    ...
    def test_check_file_max_count(self):
        ...
        self.assertFalse(form.is_valid(), msg="Nombre de fichier max. pas respecté")
        form = TestFormMax(
            {
                'media_0': TokenFactory.token(),
            }
    ...
    def test_check_file_min_max_count(self):
        ...
        self.assertFalse(form.is_valid(), msg="Nombre de fichier min. & max. pas respecté")
        form = TestFormMinMax(
            {
                'media_0': TokenFactory.token(),
                'media_1': TokenFactory.token(),
                'media_2': TokenFactory.token(),
            }
        )
    ...
    def test_persist_confirms_token(self):
        ...
        with patch('osis_document_components.services.confirm_remote_upload') as confirm_remote_upload:
            ...
            confirm_remote_upload.return_value = {"uuid": upload_uuid}
            confirmed_ids = form.fields['media'].persist(form.cleaned_data['media'], mock_instance)
            self.assertListEqual(confirmed_ids, [{"uuid": upload_uuid}])
